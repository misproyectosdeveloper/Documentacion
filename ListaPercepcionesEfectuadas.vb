Public Class ListaPercepcionesEfectuadas
    ' 
    ...
    Dim PercepcionArba As Integer
    Dim TotalArba As Decimal
    Private Sub ListaPercepciones_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Tablas.Read("SELECT Clave,Nombre,Formula FROM Tablas WHERE Tipo = 25 AND CodigoRetencion = 2;", Conexion, DtTipo) Then Me.Close() : Exit Sub
        ...
        If RowsBusqueda.Length > 0 Then
            PercepcionArba = RowsBusqueda(0).Item("Clave")
        End If
    ...
    Private Sub ButtonAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonAceptar.Click
        ...
        Dim Sql As String
        Sql = "SELECT 100 AS TipoComprobante,C.Nombre,C.Cuit,F.Liquidacion As Comprobante,CAST(FLOOR(CAST(F.FechaContable AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,D.Concepto AS MedioPago,D.Importe,0.00 AS Alicuota,F.Liquidacion AS Retencion,F.Liquidacion AS Rec FROM Proveedores AS C INNER JOIN (LiquidacionCabeza AS F INNER JOIN LiquidacionDetalleConceptos AS D ON F.Liquidacion = D.Liquidacion) ON C.Clave = F.Proveedor WHERE " & SqlFechaContable & SqlContable & _
                   " UNION ALL " & _
              "SELECT F.TipoNota AS TipoComprobante,C.Nombre,C.Cuit,F.Nota As Comprobante,CAST(FLOOR(CAST(F.Fecha AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,D.Percepcion AS MedioPago,D.Importe,D.Alicuota,D.Comprobante AS Retencion,F.Nota AS Rec FROM Proveedores AS C INNER JOIN (RecibosCabeza AS F INNER JOIN RecibosPercepciones AS D ON F.TipoNota = D.TipoComprobante AND F.Nota = D.Comprobante) ON C.Clave = F.Emisor WHERE F.FechaContable = '18000101' AND F.ChequeRechazado = 0 AND (F.TipoNota = 6 OR F.TipoNota = 8) AND " & SqlFecha & _
                   " UNION ALL " & _
              "SELECT F.TipoNota AS TipoComprobante,C.Nombre,C.Cuit,F.Nota As Comprobante,CAST(FLOOR(CAST(F.FechaContable AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,D.Percepcion AS MedioPago,D.Importe,D.Alicuota,D.Comprobante AS Retencion,F.Nota AS Rec FROM Proveedores AS C INNER JOIN (RecibosCabeza AS F INNER JOIN RecibosPercepciones AS D ON F.TipoNota = D.TipoComprobante AND F.Nota = D.Comprobante) ON C.Clave = F.Emisor WHERE F.FechaContable <> '18000101' AND F.ChequeRechazado = 0 AND (F.TipoNota = 6 OR F.TipoNota = 8) AND " & SqlFecha & _
                   " UNION ALL " & _
              "SELECT F.TipoNota AS TipoComprobante,C.Nombre,C.Cuit,F.Nota As Comprobante,CAST(FLOOR(CAST(F.Fecha AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,D.Percepcion AS MedioPago,D.Importe,D.Alicuota,D.Comprobante AS Retencion,F.Nota AS Rec FROM Clientes AS C INNER JOIN (RecibosCabeza AS F INNER JOIN RecibosPercepciones AS D ON F.TipoNota = D.TipoComprobante AND F.Nota = D.Comprobante) ON C.Clave = F.Emisor WHERE F.FechaContable = '18000101' AND F.ChequeRechazado = 0 AND (F.TipoNota = 5 OR F.TipoNota = 7) AND " & SqlFecha & _
                   " UNION ALL " & _
              "SELECT F.TipoNota AS TipoComprobante,C.Nombre,C.Cuit,F.Nota As Comprobante,CAST(FLOOR(CAST(F.FechaContable AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,D.Percepcion AS MedioPago,D.Importe,D.Alicuota,D.Comprobante AS Retencion,F.Nota AS Rec FROM Clientes AS C INNER JOIN (RecibosCabeza AS F INNER JOIN RecibosPercepciones AS D ON F.TipoNota = D.TipoComprobante AND F.Nota = D.Comprobante) ON C.Clave = F.Emisor WHERE F.FechaContable <> '18000101' AND F.ChequeRechazado = 0 AND (F.TipoNota = 5 OR F.TipoNota = 7) AND " & SqlFechaContable & _
                   " UNION ALL " & _
              "SELECT 2 AS TipoComprobante,C.Nombre,C.Cuit,F.Factura As Comprobante,CAST(FLOOR(CAST(F.FechaContable AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,P.Percepcion AS MedioPago,P.Importe,P.Alicuota,F.Factura AS Retencion,F.Factura AS Rec FROM Clientes AS C INNER JOIN (FacturasCabeza AS F INNER JOIN RecibosPercepciones AS P ON F.Factura = P.Comprobante AND P.TipoComprobante = 2) ON C.Clave = F.Cliente WHERE " & SqlFechaContable & SqlContable & _
                   " UNION ALL " & _
              "SELECT 4 AS TipoComprobante,'' AS Nombre,0 AS Cuit,F.NotaCredito As Comprobante,CAST(FLOOR(CAST(F.FechaContable AS FLOAT)) AS DATETIME) AS Fecha,F.Estado,P.Percepcion AS MedioPago,P.Importe,P.Alicuota,F.NotaCredito AS Retencion,F.NotaCredito AS Rec FROM NotasCreditoCabeza AS F INNER JOIN RecibosPercepciones AS P ON F.NotaCredito = P.Comprobante AND P.TipoComprobante = 4 WHERE " & SqlFechaContable & ";"
            ...
            If RowsBusqueda.Length <> 0 And MovimientoOk(Row) Then
                If TipoAnt <> Row("MedioPago") Then
                    If TipoAnt <> 0 Then
                        ...
                        If TipoAnt = PercepcionArba Then TotalArba = SubTotal
                        SubTotal = 0
                    End If
                ...
                Select Case Row("TipoComprobante")
                    Case 2
                        RowGrid("Cartel") = "Factura"
                    Case 4
                        RowGrid("Cartel") = "N.Credito Por Dev."
                        HallaClienteNC(Row("Comprobante"), Row("Nombre"), Row("Cuit"))
                        Row("Importe") = -Row("Importe")
                    Case 100
                        RowGrid("Cartel") = "Liquidación a Prov."   'No tiene percepciones. esta por las dudas.
                    Case 5
    ...
    Private Sub ButtonExcel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonExcel.Click
        ...
        GridAExcel(Grid, GNombreEmpresa & "   " & Date.Now, "", "Percepciones Efectuadas - PERIODO :           " & Format(DateTimeDesde.Value, "dd/MM/yyyy") & "      -      " & Format(DateTimeHasta.Value, "dd/MM/yyyy"))
    ...
    Private Sub CreaDtGrid()
        ...
        Dim Alicuota As DataColumn = New DataColumn("Alicuota")
    ...
    Private Sub ArmaTipoComprobante()
        ...
        Row("Nombre") = "N.Credito por Dev."
        Row("Codigo") = 4
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
        Row("Nombre") = "Liqui. a Pov."
        Row("Codigo") = 100
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
        Row("Nombre") = "N.D. a Clien."
        Row("Codigo") = 5
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
        Row("Nombre") = "N.C. a Clien."
        Row("Codigo") = 7
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
        Row("Nombre") = "N.D a Prov."
        Row("Codigo") = 6
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
        Row("Nombre") = "N.C. a Prov."
        Row("Codigo") = 8
        DtTipoComprobante.Rows.Add(Row)
        '
        Row = DtTipoComprobante.NewRow
    ...
    End Sub
    Private Sub HallaClienteNC(ByVal NotaCredito As Decimal, ByRef Nombre As String, ByRef Cuit As Decimal)

        Dim Dt As New DataTable
        Dim Sql As String = "SELECT C.Nombre,C.Cuit FROM Clientes AS C INNER JOIN (FacturasCabeza AS F INNER JOIN NotasCreditoCabeza AS N ON F.Factura = N.Factura) ON C.Clave = F.Cliente WHERE N.NotaCredito = " & NotaCredito & ";"
    ...
    Private Sub Grid_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Grid.CellDoubleClick
        ...
        Select Case Grid.CurrentRow.Cells("TipoComprobante").Value
            Case 2
                UnaFactura.PFactura = Grid.CurrentRow.Cells("Rec").Value
                UnaFactura.PAbierto = True
                UnaFactura.PBloqueaFunciones = True
                UnaFactura.ShowDialog()
            Case 4
                UnaNotaCredito.PNota = Grid.CurrentRow.Cells("Rec").Value
