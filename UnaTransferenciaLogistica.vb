Public Class UnaTransferenciaLogistica
    ...
    End Sub
    Private Sub UnaTransferenciaLogistica_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub ButtonAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonAceptar.Click
        ...
        Using MiConexion As New OleDb.OleDbConnection(ConexionTrans)
            Try
                ...
                Try
                    '
                    Using DaP As New OleDb.OleDbDataAdapter("SELECT * FROM TransLogisticasCabeza;", MiConexion)
                        DaP.SelectCommand.Transaction = Trans
                        Dim SqlCommandBuilder As New OleDb.OleDbCommandBuilder(DaP)
                        DaP.Update(DtCabezaW)
                    End Using
                    Using DaP As New OleDb.OleDbDataAdapter("SELECT * FROM TransLogisticasDetalle;", MiConexion)
                        DaP.SelectCommand.Transaction = Trans
                        Dim SqlCommandBuilder As New OleDb.OleDbCommandBuilder(DaP)
                        DaP.Update(DtDetalleW)
                    End Using
                    '
                    Using DaP As New OleDb.OleDbDataAdapter("SELECT * FROM StockArticulosLogisticos;", MiConexion)
                        DaP.SelectCommand.Transaction = Trans
                        Dim SqlCommandBuilder As New OleDb.OleDbCommandBuilder(DaP)
                        DaP.Update(DtStock)
                    End Using
    ...
    Private Sub ArmaArchivos()
        ...
        If Not Tablas.Read("SELECT * FROM TransLogisticasCabeza WHERE Transferencia = " & PTrans & ";", ConexionTrans, DtTransCabeza) Then Me.Close() : Exit Sub
        ...
        If Not Tablas.Read("SELECT * FROM TransLogisticasDetalle WHERE Transferencia = " & PTrans & ";", ConexionTrans, DtTransDetalle) Then Me.Close() : Exit Sub
    ...
    Private Function ActualizaArchivoAlta(ByVal Dt As DataTable) As Boolean
        ...
        For Each Row As DataRow In DtGrid.Rows
            If Not Row.RowState = DataRowState.Deleted Then
                Sql = "SELECT * FROM StockArticulosLogisticos WHERE Articulo = " & Row("Articulo") & " AND Deposito = " & ComboDepositoOrigen.SelectedValue & ";"
                If Not Tablas.Read(Sql, ConexionTrans, Dt) Then Return False
            End If
        Next
        For Each Row As DataRow In DtGrid.Rows
            If Not Row.RowState = DataRowState.Deleted Then
                RowsBusqueda = Dt.Select("Articulo = " & Row("Articulo") & " AND Deposito = " & ComboDepositoOrigen.SelectedValue)
                If RowsBusqueda.Length = 0 Then
                    MsgBox("No Hay Stock en Deposito Origen para el Articulo " & NombreArticuloLogistico(Row("Articulo")), MsgBoxStyle.Critical)
                    Return False
                End If
                RowsBusqueda(0).Item("Stock") = CDec(RowsBusqueda(0).Item("Stock")) - Row("Cantidad")
                If RowsBusqueda(0).Item("Stock") < 0 Then
                    MsgBox("Stock Negativo Para el Articulo " & NombreArticuloLogistico(Row("Articulo")) & " Operación se CANCELA.", MsgBoxStyle.Critical + MsgBoxStyle.DefaultButton3, "Error")
                    Return False
                End If
            End If
        Next
        ...
        For Each Row As DataRow In DtGrid.Rows
            If Not Row.RowState = DataRowState.Deleted Then
                Sql = "SELECT * FROM StockArticulosLogisticos WHERE Articulo = " & Row("Articulo") & " AND Deposito = " & ComboDepositoDestino.SelectedValue & ";"
                If Not Tablas.Read(Sql, ConexionTrans, Dt) Then Return False
            End If
        Next
        For Each Row As DataRow In DtGrid.Rows
            If Not Row.RowState = DataRowState.Deleted Then
                RowsBusqueda = Dt.Select("Articulo = " & Row("Articulo") & " AND Deposito = " & ComboDepositoDestino.SelectedValue)
                If RowsBusqueda.Length <> 0 Then
    ...
    Private Sub LlenaCombosGrid()
        ...
        Articulo.DataSource = Tablas.Leer("SELECT Clave,Nombre FROM Tablas WHERE Tipo = 6 ORDER BY Nombre;")
    ...
    End Sub
    Private Function NombreArticuloLogistico(ByVal Clave As Integer) As String

        Try
            Using Miconexion As New OleDb.OleDbConnection(Conexion)
                Miconexion.Open()
                Using Cmd As New OleDb.OleDbCommand("SELECT Nombre FROM Tablas WHERE Tipo = 6 AND Clave = " & Clave & ";", Miconexion)
                    Return Cmd.ExecuteScalar()
                End Using
            End Using
        Catch ex As Exception
            MsgBox("Error Base de Datos al leer Tabla: TransLogisticas.")
            End
        End Try
